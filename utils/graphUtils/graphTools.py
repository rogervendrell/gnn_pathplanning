...
def computeNeighborhood(S, K, N='all', nb='all', outputType='list'):
    ...
    if nb == 'all':
        nb = S.shape[0]
    # Cannot return more nodes than there are
    assert N >= 0 and N <= S.shape[0]
...
def createGraph(graphType, N, graphOptions):
    ...
    if graphType == 'SBM':
        ...
        # Check that the number of communities is an integer
        assert int(C) == C
            ...
            for n in range(N):  # Go through all nodes in order
                ...
                if len(inds_equallyfar) == 1:  # if a unique farthest node to
                    ...
                    r = np.random.permutation(
                        len(inds_equallyfar)).astype(np.int)
        ...
        # if True, isolated nodes
        isolatedNodes = graphOptions['isolatedNodes']
